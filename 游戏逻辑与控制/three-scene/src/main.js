...
const TRAIL_SPAWN_INTERVAL_FRAMES = 2; // 略密集，让长拖尾连贯
const TRAIL_MAX_COUNT = 16;            // 给松开长拖尾更多预算
...
const GESTURE_SENSITIVITY_Y = 4.0; // 原写死 4.0，同上
...
const LEVELS = [
  {
    ...
    pieceLore: {
      '大斗':       { brief: '【待补充：大斗简介】' },
      '华拱':       { brief: '【待补充：单翘简介】' },
      '正心瓜拱':   { brief: '【待补充：正心瓜拱简介】' },
      '散科-左边':  { brief: '【待补充：十八斗简介】' },
      '散科-右边':  { brief: '【待补充：槽升子简介】' },
    },
    finishLore: {
      title: '单翘单昂平身科',
      subtitle: '一翘三升 · 清代官式木构精髓',
      body: '【待补充：单翘单昂平身科的形制、年代、出处、文化意义等正式介绍文字】',
      source: '',
    },
  ...
  },
  {
    id: 2,
    name: '北京紫禁城御花园万春亭斗拱',
    ghostPath: '/models/level2/幽灵组件.glb',     // ⚠️ 待补：整体成品 GLB（半透明叠在拼接区作目标参照）
    showcasePath: '/models/level2/成品.glb',      // ⚠️ 待补：实体成品 GLB（摆在拼接区旁作展示）
    showcasePosition: { x: -3.5, y: 0, z: 0 },    // 实体成品摆位（拼接区左侧）
    piecePathPrefix: '/models/level2/',
    pieceNames: [
      // ⚠️ 待补：6 个零件 GLB 文件名（去掉 .glb 后缀）
      'piece1', 'piece2', 'piece3', 'piece4', 'piece5', 'piece6',
    ],
    displayNames: {
      'piece1': '【待补】', 'piece2': '【待补】', 'piece3': '【待补】',
      'piece4': '【待补】', 'piece5': '【待补】', 'piece6': '【待补】',
    },
    pieceLore: {},
    finishLore: {
      title: '万春亭斗拱',
      subtitle: '紫禁城御花园 · 皇家攒尖顶杰作',
      body: '【待补充：北京故宫御花园万春亭斗拱的形制、清代官式建筑特征、文化意义】',
      source: '',
    },
    manualAssemblyOrder: null,
    // 整体建模成品没有"配套"mesh，必须用 tools/calibrator.html 校准后把 JSON 粘到这里
    // 格式：{ '零件名': { position: {x,y,z}, quaternion: {x,y,z,w} }, ... }
    targetOverrides: null,
    ghostAlwaysVisible: true, // 第二关 ghost 半透明常驻，给玩家"目标位置"视觉
    ghostOpacity: 0.35,       // 比 hint 提示稍暗，避免和实体成品视觉冲突
  },
];
...
function applyLevelConfig(index) {
  ...
  console.log(`[Level] 应用配置: ${currentLevel.name} (id=${currentLevel.id})`);
}
...
async function init() {
  ...
  // 记录默认机位（"复位 V" 按钮会平滑动画回到这里）
  DEFAULT_CAMERA_POS.copy(startPos);
        ...
        if (currentLevel.targetOverrides) {
          ...
          for (const [baseName, t] of Object.entries(currentLevel.targetOverrides)) {
            TARGET_CONFIG[baseName] = {
              ...
              isOccupied: false,
              originalNode: model, // 用整体模型当 originalNode（hint 预览会克隆它，可接受）
            };
            console.log(`[Main] ✅ 注入目标位置（人工校准）: ${baseName}`, t.position);
          }
        } else {
          model.traverse((node) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      gltfLoader.load(
        ...
        (gltf) => {
          ...
          // ✅ 备料架布局 — 恢复原版位置，只把正心瓜拱挪近华拱（远离 MediaPipe 边缘）
          // 后排（Z=0.5）：大斗 X=2.0, 华拱 X=3.2, 正心瓜拱 X=3.8（原 4.4 挪近华拱 0.6m）
          // 前排（Z=1.0）：散科-左 X=1.5, 散科-右 X=3.0
          let dockPos;
  ...
  return new Promise((resolve) => {
    ...
    gltfLoader.load(
      ...
      (gltf) => {
        ...
        console.log(`[Showcase] ✅ 实体成品已就位 @ (${sp.x}, ${sp.y}, ${sp.z})`);
        resolve();
      },
      undefined,
      (err) => {
...
async function loadLevel(index) {
  ...
  // 6. 加载新模型（顺序与 init 一致：ghost → buildOrder → pieces → showcase）
  try {
    await loadGhostReference();
    buildAssemblyOrder();
    await loadInteractivePieces();
...
function showHintPreview() {
  hideHintPreview(); // 先清掉旧的

  const remaining = getCurrentStepPieces();
  if (remaining.length === 0) return; // 已全部完成

  // 同层多构件时，只预览第一个，避免视觉杂乱
  const partID = remaining[0];
  const target = TARGET_CONFIG[partID];
  if (!target || !target.originalNode) return;

  const sourceNode = target.originalNode;

  // 拿到源 mesh 的世界缩放（target 里只存了 position/quaternion）
  const worldScale = new THREE.Vector3();
  sourceNode.getWorldScale(worldScale);

  // 创建独立的提示 mesh（共享 geometry，新材质）
  ...
  console.log(`[Hint] 显示目标预览: ${partID}`);
}
...
/**
 * 切换知识图浮层（按 K 或点知识图按钮）
 * 点击浮层任意位置或再按 K 关闭；图片路径 /第一场景知识图.jpg 由用户放到 public/
 */
function toggleKnowledgeImage() {
  ...
  img.onerror = () => {
    ...
    tip.innerHTML = '⚠️ 知识图未找到<br><br>请把 <code style="color:#ffd24a;">第一场景知识图.jpg</code> 放到 <code style="color:#ffd24a;">public/</code> 目录后刷新';
...
function hideProximityHint() {
  ...
  proximityMesh.material.dispose();
  proximityMesh = null;
}
  ...
  if (handInput && handInput.isConnected()) {
    ...
    if (gesture.isPinching) {
      ...
      if (!isGrabbing) {
        ...
        const GRAB_RADIUS_NDC = 0.15; // NDC 阈值（约 15% 屏幕高度，与屏幕分辨率无关）
